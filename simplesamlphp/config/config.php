...
$config = array (
	 ...
	 */
	'secretsalt' => 'fygtbfqe1oo4z4zp9c$uxib6p2ky10ux',
	 ...
	 */
	'technicalcontact_name'     => 'John Kealy',
	'technicalcontact_email'    => 'john.kealy@ucsf.edu',
...
/* commented out old acquia code (jk-2014-02-05)
// Defines account specific settings.
if (!defined('AH_ACCOUNT_NAME')) {
  define('AH_ACCOUNT_NAME', 'ucsfp');
  define('AH_DATABASE_NAME', 'ucsfp');
}
...
if ('local' != $_ENV['AH_SITE_ENVIRONMENT']) {
  ...
  $config['store.type'] = 'sql';
  $config['store.sql.dsn'] = 'mysql:host=127.0.0.1;port=33066;dbname=simplesaml';
  $config['store.sql.username'] = 'drupaluser';
  $config['store.sql.password'] = '';
  $config['store.sql.prefix'] = 'simplesaml';
}
// end commmented out code (jk 2014-02-05)
*/

// added new acquia code (jk 2014-02-05)
// see https://gist.github.com/acquiacat/8046522
// ticket #15066-111279
...
$ah_database_name = 'ucsfp';

// Set some security and other configs that are set above, however we
// overwrite them here to keep all changes in one area
$config['technicalcontact_name'] = "John Kealy";
$config['technicalcontact_email'] = "john.kealy@ucsf.edu";

// Change these for your installation
$config['secretsalt'] = 'fygtbfqe1oo4z4zp9c$uxib6p2ky10ux';
$config['auth.adminpassword'] = 'Web@dmin';
...
if ($_ENV['AH_SITE_ENVIRONMENT'] != 'local') {
  ...
  // FALLBACK:
  // assume a non-Acquia environment, e.g. a local development machine.
  // load a local config file if it exists.
  // [ST 2014/05/02]
  $localConfig = dirname(__FILE__) . '/local.config.php';
