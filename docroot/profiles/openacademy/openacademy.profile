...
function openacademy_install_tasks_alter(&$tasks, $install_state) {
  ...
  $tasks['install_select_locale']['function'] = 'openacademy_locale_selection';

  // Create a more fun finished page with our Open Academy Saurus
  $tasks['install_finished']['function'] = 'openacademy_finished_yah';
 ...
 */
function openacademy_locale_selection(&$install_state) {
 ...
 */
function openacademy_set_theme() {
  $themes = list_themes();
  $theme = $themes['openacademy_maintenance'];
 ...
 */
function openacademy_apps_check($form, &$form_state) {
  $form = array();
...
function openacademy_theme_form($form, &$form_state) {
  ...
  foreach(system_rebuild_theme_data() as $theme) {
    if (!in_array($theme->name, array('test_theme', 'update_test_basetheme', 'update_test_subtheme', 'block_test_theme', 'stark', 'seven', 'openacademy_maintenance'))) {
